...
//            NangmanLetter nangmanLetter = nangmanLetterBoxService.writeAndSendLetter(request, senderNickname);
    ...
    public ApiResponse<NangmanLetterBoxResponseDTO.SelectedLetterResultDTO> getNangmanLetterInfo(
        ...
        try {
            ...
            //응답 생성
            NangmanLetterBoxResponseDTO.SelectedLetterResultDTO selectedLetterResultDTO = NangmanLetterBoxConverter.toSelectedLetterResultDTO(
                    seledtedLetter, randomNickname);
    ...
    public ApiResponse<NangmanLetterBoxResponseDTO.PreviewReplyResultDTO> getPreviewReplyForLetter(
            @PathVariable Long nangmanLetterId) {
        try {
            // 현재 로그인된 사용자의 ID 또는 정보를 얻어온다고 가정
            // SecurityContextHolder에서 현재 사용자의 정보를 가져오는 방법
            ...
            NangmanLetterBoxResponseDTO.PreviewReplyResultDTO previewReplyResultDTO = replyOptional.map(
                            NangmanLetterBoxConverter::toPreviewReplyResultDTO)
    ...
    public ApiResponse<List<NangmanLetterBoxResponseDTO.PreviewBothResultDTO>> getMyNangmanReplies() {
        ...
        // 사용자가 답장한 목록 조회
        List<NangmanLetterBoxResponseDTO.PreviewBothResultDTO> userReplyList = nangmanLetterBoxService.getReplyListByUserId(
                userId);
